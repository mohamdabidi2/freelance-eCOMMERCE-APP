...
class ProductsTemps extends Component {
    constructor(props) {
        super(props);
        this.state = { 
            products:[],
    ...
    render() { 
        return ( <div>
            ...
            <div className="prodycti">
                ...
                {this.state.products.filter(el=>el.prix>=Number(this.state.min)&&el.ProductName.toUpperCase().includes(this.props.match.params.search.toUpperCase())&& el.category.toUpperCase().includes(this.state.qte.toUpperCase())&&el.prix<=Number(this.state.max))
                .map(el=>{return(
...
export default withRouter( ProductsTemps);